...
public class TradRBBalancer<K extends Comparable<K>, V> extends RBBalancer<K, V>  {
    public BSTMap<K, V, RBInfo<K, V>>.Node putFixup(BSTMap<K, V, RBInfo<K, V>>.Node fix) {
    	...
    	if (fix.getInfo().isRedL() && fix.getInfo().isRedR() && fix.getInfo().isRedRL()) reddenRoot(fix); // right-left red uncle
        else if (fix.getInfo().isRedL() && fix.getInfo().isRedR() && fix.getInfo().isRedRR()) reddenRoot(fix); // right-right red uncle
        else if (!fix.getInfo().isRedL() && fix.getInfo().isRedR() && fix.getInfo().isRedRL()) { // right-left black uncle
        ...
        } else if (!fix.getInfo().isRedL() && fix.getInfo().isRedR() && fix.getInfo().isRedRR()) { // right-right black uncle
            fix.getInfo().redden();
            fix.getRight().getInfo().blacken();
            fix.getInfo().recompute();
            fix.getRight().getInfo().recompute();
            fix = fix.rotateLeft();
        } else if (fix.getInfo().isRedR() && fix.getInfo().isRedL() && fix.getInfo().isRedLR()) { // left-right red uncle
    		reddenRoot(fix);
        } else if (fix.getInfo().isRedR() && fix.getInfo().isRedL() && fix.getInfo().isRedLL()) { // left-left red uncle
    		reddenRoot(fix);
        } else if (!fix.getInfo().isRedR() && fix.getInfo().isRedL() && fix.getInfo().isRedLR()) { // left-right black uncle
            fix.leftRotateLeft();
            fix.getInfo().redden();
            fix.getInfo().recompute();
            ...
            fix = fix.rotateRight();
        } else if (!fix.getInfo().isRedR() && fix.getInfo().isRedL() && fix.getInfo().isRedLL()) { // left-left black uncle
            fix.getInfo().redden();
            fix.getInfo().recompute();
            fix.getLeft().getInfo().blacken();
            fix.getLeft().getInfo().recompute();
    ...
    private void reddenRoot(BSTMap<K, V, RBInfo<K, V>>.Node fix) {
        fix.getInfo().redden();
        fix.getLeft().getInfo().blacken();
        fix.getRight().getInfo().blacken();
        fix.getLeft().getInfo().recompute();
