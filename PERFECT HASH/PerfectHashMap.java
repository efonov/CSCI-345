...
public class PerfectHashMap<K, V> implements Map<K, V> {
    ...
    private class SecondaryMap implements Map<K, V> {
		...
		SecondaryMap(Set<K> givenKeys) {
        	...
        	while(true) {
            	boolean goal = true;
        		for(K keyi : givenKeys) {
        			for(K keyj : givenKeys) {
        				if(!keyi.equals(keyj) && h.hash(keyi) == h.hash(keyj)) goal = false;
        			}
        		}
        		if(goal) {
        			break;
       ...
       public Iterator<K> iterator() { 
    	   int i = 0;        	
    	   //else return an iterator over the keys
    	   while(i < keys.length && keys[i] == null) {
    		   i++;
    	   }

    	   final int firstLoc = i;
    	   ...
    	   return new Iterator<K>(){
    		   ...
    		   public K next() {
    			   K toReturn = keys[i];
    			   System.out.println(toReturn + " is at: " + i + "/" + keys.length);
    			   i++;
    			   while(i < keys.length && keys[i] == null) {
    				   i++;
    			   }
	...
	public PerfectHashMap(K[] keys) {
    	//greatest prime number greater than all keys
    	p  = PrimeSource.nextOrEqPrime(findMaskAndGreatestKey(keys));
    	m = keys.length;
    	h = HashFactory.universalHashFunction(p, m, mask);
    ...
    */
    ...
    public Iterator<K> iterator() {
    	...
    	int i = 0;
    	//find the first bucket with keys
    	while(i < secondaries.length && !secondaries[i].iterator().hasNext()) {
    		i++;
    	}
    	final int firstBucket = i;
    	
    	//a dummy iterator to handle an empty Iterator case
    	if(firstBucket == secondaries.length) 
    		return new Iterator<K>() {
			public boolean hasNext() {
				return false;
			}
    	...
    	return new Iterator<K>() {
    		int i = firstBucket;
    		//a secondary iterator over the bucket
    		Iterator<K> subIt = secondaries[i].iterator();
    		
    		//if we reach the last bucket there are no more keys
    		public boolean hasNext() {
    			return i < secondaries.length;
    		}
    		
    		//the next function that uses the secondary iterator
            public K next() {
				K toReturn = subIt.next();
				//if there are no more keys in this button find the next bucket with keys
				if(!subIt.hasNext()) {
					i++;
			    	while(i < secondaries.length && !secondaries[i].iterator().hasNext()) {
			    		i++;
			    	...
			    	//set the iterator to iterate over this next bucket
			    	subIt = secondaries[i].iterator();
