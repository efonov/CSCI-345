...
public class BitVecNSet implements NSet {
    ...
    public boolean contains(Integer item) {
    	...
    	int mask = item % 8;
    	boolean result;
    	if((internal[index] & (1 << mask)) == 0) result = false;
    	else result = true;
    	return result;
    }
    ...
    public NSet union(NSet other) {
        ...
        BitVecNSet toReturn = new BitVecNSet(range);
        for(int i = 0; i < range; i++) {
        	if(other.contains(i) || contains(i)) toReturn.add(i);
        }
        return toReturn;
    }
    ...
    public NSet difference(NSet other) {
        ...
        BitVecNSet toReturn = new BitVecNSet(range);
        for(int i = 0; i < range; i++) {
        	if(contains(i) && !other.contains(i)) toReturn.add(i);
        }
        return toReturn;
    }
    ...
    public int size() {
    	int size = 0;
    	for(int i = 0; i < internal.length*8; i++) {
    		if(contains(i))size++;
    	}
    	return size;
    }
    ...
    public Iterator<Integer> iterator() {
    	int j = 0;
        while (j < internal.length*8 && !contains(j)) j++;
        final int finalJ = j;
		return new Iterator<Integer>() {
			
        	int current = finalJ;
			public boolean hasNext() {
                return current < internal.length*8;
			}
			...
			public Integer next() {
				...
				current++;
				while (current < internal.length*8 && !contains(current)) current++;
				return toReturn;
			}
