...
public class OpenAddressingHashMap<K, V> implements Map<K, V> {
    ...
    public void put(K key, V val) {
    	...
    	if(containsKey(key)) {
    		...
    		while(!done) {
        		if(table[index] == null || table[index] == deleted) {
        			table[index] = new Pair<K, V>(key, val);
        			numPairs++;
        			done = true;
        		}
            	else {
    	...
    	if(((double)(numPairs+deleteds))/table.length > loadFactor) {
    		rehash();
    	}
    }
    ...
    synchronized private void rehash() {
        ...
        Pair<K,V>[] newTable = new Pair[prober.resize(table.length)];
    ...
    public Iterator<K> iterator() {
    	...
    	int firstLoc = 0;
    	if(numPairs>0) {
        	while(table[firstLoc] == null || table[firstLoc] == deleted) {
        		firstLoc++;
    		};
    	}
    	else firstLoc = table.length;
		final int i = firstLoc;
    	...
    	return new Iterator<K>() {
            ...
            public K next() {
            	...
            	current++;
            		while(current < table.length &&(table[current] == null || table[current] == deleted)) {
            		current++;
            	}
                return toReturn;
            }
